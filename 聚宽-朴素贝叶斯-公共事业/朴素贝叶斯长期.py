...
def handle_data(context, data):
    ...
    for security in security_order:
        ...
        for security in buying:
            if gain>1.05:
                order_target_value(security,init_cash*0.7/len(buying))
                log.info("Buying %s" % (security))
            elif gain<1.05 and gain>1:
                order_target_value(security,init_cash*0.1/len(buying))
                log.info("Buying %s" % (security))
                

    for security in g.stocks:
        if gain<0.97 and context.portfolio.positions[security].closeable_amount > 0:        
