...
def handle_data(context):                  # 每个交易日的买入卖出指令
    ...
    for index in universe:
        ...
        if index in hotStock:
            if gain>=1.05:
                amount=amount = 0.7*account.cash/len(hotStock)/context.current_price(index)
                account.order(index,amount)
            elif gain<1.05 and gain>1:
                amount=amount = 0.1*account.cash/len(hotStock)/context.current_price(index)
                account.order(index,amount)
        else:
